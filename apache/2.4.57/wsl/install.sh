...
if [ ! -e $HOME/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/httpd.conf ]; then
	printf "httpd.conf not found\n"
	exit
fi

if [ ! -e $HOME/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/httpd-vhosts.conf ]; then
...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/apachectl" ]; then
	...
	if [ -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/apachectl" ]; then
		...
		mv conf/httpd.conf ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/httpd.conf.default
		ln -s ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/httpd.conf conf/httpd.conf
		mv conf/extra/httpd-vhosts.conf ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/httpd-vhosts.conf.default
		ln -s ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/httpd-vhosts.conf conf/extra/httpd-vhosts.conf
