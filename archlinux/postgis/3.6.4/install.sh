...
if [ ! -e "$HOME/programs/$POSTGRES_FOLDER_NAME/$POSTGRES_VERSION/lib/postgis-3.so" ]; then
	...
	wget --show-progress -c --tries=10 --timeout=30 --retry-connrefused --waitretry=5 "https://postgis.net/stuff/$ARCHIVE_FILE" > $HOME/logs/${FOLDER_NAME}/${VERSION}/download.txt 2>&1
