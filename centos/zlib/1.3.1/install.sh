...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/lib/libz.so" ]; then
	...
	wget --show-progress -c --tries=10 --timeout=30 --retry-connrefused --waitretry=5 "https://www.zlib.net/fossils/$ARCHIVE_FILE" > $HOME/logs/$FOLDER_NAME/$VERSION/download.txt 2>&1
