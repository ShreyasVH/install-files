...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/start.sh" ]; then
	...
	cp $HOME/workspace/myProjects/config-samples/$OS/$FOLDER_NAME/$VERSION/elasticsearch.yml elasticsearch.yml

	print_message "${bold}${green}Building image${clear}"
	docker compose -f docker-compose.yml build > /dev/null 2>&1

	touch start.sh
	echo 'PORT=$(grep '\''http.port: '\'' elasticsearch.yml | awk '\''{print $2}'\'')' >> start.sh
	echo '' >> start.sh
	echo 'if ! lsof -i :$PORT > /dev/null; then' >> start.sh
	echo -e '\techo "Starting"' >> start.sh
	echo -e "\tdocker compose -p elasticsearch -f docker-compose.yml up -d > /dev/null 2>&1" >> start.sh
	echo 'fi' >> start.sh

	touch stop.sh
	echo 'PORT=$(grep '\''http.port: '\'' elasticsearch.yml | awk '\''{print $2}'\'')' >> stop.sh
	echo '' >> stop.sh
	echo 'if lsof -i :$PORT > /dev/null; then' >> stop.sh
	echo -e '\techo "Stopping"' >> stop.sh
	echo -e "\tdocker compose -p elasticsearch -f docker-compose.yml stop > /dev/null 2>&1" >> stop.sh
	echo 'fi' >> stop.sh
fi