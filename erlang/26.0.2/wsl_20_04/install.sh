...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/erl" ]; then
	...
	bash $INSTALL_FILES_DIR/$FOLDER_NAME_PERL/$PERL_VERSION/wsl_20_04/install.sh
	bash $INSTALL_FILES_DIR/$FOLDER_NAME_OPENSSL/$OPENSSL_VERSION/wsl_20_04/install.sh
	bash $INSTALL_FILES_DIR/$NCURSES_FOLDER_NAME/$NCURSES_VERSION/wsl_20_04/install.sh
