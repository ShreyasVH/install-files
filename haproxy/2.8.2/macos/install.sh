...
if [ ! -d "$HOME/programs/$FOLDER_NAME/$VERSION" ]; then
	...
	cd $VERSION
	make PREFIX=$HOME/programs/$FOLDER_NAME/$VERSION TARGET=osx USE_OPENSSL=1 SSL_INC=$HOME/programs/$OPENSSL_FOLDER_NAME/$OPENSSL_VERSION/include SSL_LIB=$HOME/programs/$OPENSSL_FOLDER_NAME/$OPENSSL_VERSION/lib
	...
	touch start.sh
	echo "sudo haproxy -f ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/macos/haproxy.cfg -D" >> start.sh
