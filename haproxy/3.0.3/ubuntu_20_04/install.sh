...
if [ ! -e $HOME/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/ubuntu_20_04/haproxy.cfg ]; then
...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/sbin/haproxy" ]; then
	...
	bash $INSTALL_FILES_DIR/$OPENSSL_FOLDER_NAME/$OPENSSL_VERSION/ubuntu_20_04/install.sh
	...
	if [ -e "$HOME/programs/$FOLDER_NAME/$VERSION/sbin/haproxy" ]; then
		...
		echo '' >> start.sh
		echo 'echo "'$USER_PASSWORD'" | sudo -S -p "" LD_LIBRARY_PATH="$LD_LIBRARY_PATH" ./sbin/haproxy -f ~/workspace/myProjects/config-samples/'$FOLDER_NAME'/'$VERSION'/ubuntu_20_04/haproxy.cfg -D' >> start.sh
