...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/lib/libgeos.so" ]; then
	...
	wget --show-progress -c --tries=10 --timeout=30 --retry-connrefused --waitretry=5 "https://download.osgeo.org/geos/$ARCHIVE_FILE" > $HOME/logs/$FOLDER_NAME/$VERSION/download.txt 2>&1
	...
	# ln -s $HOME/programs/$FOLDER_NAME/$VERSION/lib64 $HOME/programs/$FOLDER_NAME/$VERSION/lib
	if [ -e "$HOME/programs/$FOLDER_NAME/$VERSION/lib/libgeos.so" ]; then
