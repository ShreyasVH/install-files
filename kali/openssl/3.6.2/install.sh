...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/openssl" ]; then
	...
	curl -fL --retry 10 --retry-delay 5 --retry-all-errors --connect-timeout 30 -C - -O  "https://github.com/openssl/openssl/releases/download/openssl-$VERSION/$ARCHIVE_FILE" > $HOME/logs/${FOLDER_NAME}/${VERSION}/download.txt 2>&1
