...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/diff" ]; then
	...
	ARCHIVE_FILE="diffutils-$VERSION.tar.gz"
	wget -q "https://ftp.rediris.es/mirror/GNU/diffutils/$ARCHIVE_FILE"
	print_message "${bold}${green}Extracting source code${clear}" $((DEPTH))
	tar -xf $ARCHIVE_FILE
	mv "diffutils-$VERSION" $VERSION
	cd $VERSION
