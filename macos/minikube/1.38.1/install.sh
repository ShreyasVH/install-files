...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/minikube" ]; then
	...
	ARCHIVE_FILE="minikube-darwin-arm64"
	...
	SUDO_ASKPASS=$HOME/askpass.sh sudo -A install minikube-darwin-arm64 $HOME/programs/$FOLDER_NAME/$VERSION/bin/minikube > $HOME/logs/$FOLDER_NAME/$VERSION/install.txt 2>&1
