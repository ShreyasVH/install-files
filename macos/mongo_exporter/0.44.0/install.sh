...
if [ ! -d "$HOME/programs/$FOLDER_NAME/$VERSION" ]; then
	...
	ARCHIVE_FILE="mongodb_exporter-${VERSION}.darwin-arm64.tar.gz"
	wget --show-progress "https://github.com/percona/mongodb_exporter/releases/download/v${VERSION}/$ARCHIVE_FILE" > $HOME/logs/$FOLDER_NAME/$VERSION/download.txt 2>&1
	print_message "${bold}${green}Extracting source code${clear}" $((DEPTH))
	tar -xf $ARCHIVE_FILE
	mv mongodb_exporter-${VERSION}.darwin-arm64 $VERSION
	cd $VERSION
	...
	echo -e "\tmongodb_exporter --mongodb.uri=\"mongodb://localhost:${MONGO_PORT}\" --web.listen-address=\":${PORT}\" --collect-all > exporter.log 2>&1 &" >> start.sh
	echo 'fi' >> start.sh
