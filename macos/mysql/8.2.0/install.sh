...
if [ ! -d "$HOME/programs/$FOLDER_NAME/$VERSION" ]; then
	...
	ARCHIVE_FILE=mysql-$VERSION-macos13-arm64.tar.gz
	wget -q "https://downloads.mysql.com/archives/get/p/23/file/$ARCHIVE_FILE"
	print_message "${bold}${green}Extracting source code${clear}" $((DEPTH))
	tar -xf $ARCHIVE_FILE
	mv mysql-$VERSION-macos13-arm64 $VERSION
	cd $VERSION
