...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/openssl" ]; then
	...
	curl -s -OL "https://github.com/openssl/openssl/releases/download/OpenSSL_1_1_1/$ARCHIVE_FILE"
	...
	./Configure darwin64-arm64-cc --prefix=$HOME/programs/openssl/$VERSION --libdir=lib shared zlib-dynamic > $HOME/logs/$FOLDER_NAME/$VERSION/configureOutput.txt 2>&1
