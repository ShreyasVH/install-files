...
RABBITMQADMIN_FOLDER_NAME=rabbitmqadmin
RABBITMQADMIN_VERSION=$(cat "$VERSION_MAP_PATH" | jq -r --arg folder "$FOLDER_NAME" --arg version "$VERSION" --arg name "$RABBITMQADMIN_FOLDER_NAME" '.[$folder][$version][$name]')
...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/sbin/rabbitmq-server" ]; then
	...
	bash $INSTALL_FILES_DIR/$OS/$RABBITMQADMIN_FOLDER_NAME/$RABBITMQADMIN_VERSION/install.sh $((DEPTH+1))
	...
	export PATH=$HOME/programs/$RABBITMQADMIN_FOLDER_NAME/$RABBITMQADMIN_VERSION/bin:$PATH
	...
	rabbitmq-plugins enable rabbitmq_management > $HOME/logs/$FOLDER_NAME/$VERSION/managementEnable.txt 2>&1
	...
	DOMAIN_NAME=rmq_$VERSION_STRING.local.com
