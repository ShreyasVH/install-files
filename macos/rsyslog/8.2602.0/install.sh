...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/sbin/rsyslogd" ]; then
	...
	# export PKG_CONFIG_PATH=$HOME/programs/$LIBGCRYPT_FOLDER_NAME/$LIBGCRYPT_VERSION/lib/pkgconfig:$PKG_CONFIG_PATH
	# export CFLAGS=$(pkg-config --cflags libgcrypt)
	# export LDFLAGS=$(pkg-config --libs libgcrypt)
	...
	./configure --disable-libgcrypt --disable-impstats-push --prefix=$HOME/programs/$FOLDER_NAME/$VERSION > $HOME/logs/$FOLDER_NAME/$VERSION/configureOutput.txt 2>&1
