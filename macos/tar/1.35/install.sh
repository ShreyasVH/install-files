...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/tar" ]; then
	...
	./configure --prefix=$HOME/programs/$FOLDER_NAME/$VERSION --with-libiconv-prefix=$HOME/programs/$LIBICONV_FOLDER_NAME/$LIBICONV_VERSION --with-gzip=$HOME/programs/$GZIP_FOLDER_NAME/$GZIP_VERSION/bin/gzip > $HOME/logs/$FOLDER_NAME/$VERSION/configureOutput.txt 2>&1
