...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/xz" ]; then
	...
	ARCHIVE_FILE="xz-$VERSION.tar.gz"
	wget --show-progress "https://tukaani.org/xz/$ARCHIVE_FILE" > $HOME/logs/$FOLDER_NAME/$VERSION/download.txt 2>&1
	print_message "${bold}${green}Extracting source code${clear}" $((DEPTH))
	tar -xf $ARCHIVE_FILE
	mv "xz-$VERSION" $VERSION
	cd $VERSION
