...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/zsh" ]; then
	...
	ARCHIVE_FILE="zsh-$VERSION.tar.xz"
	wget --show-progress "https://www.zsh.org/pub/$ARCHIVE_FILE" > $HOME/logs/$FOLDER_NAME/$VERSION/download.txt 2>&1
	print_message "${bold}${green}Extracting source code${clear}" $((DEPTH))
	tar -xf $ARCHIVE_FILE
	mv "zsh-$VERSION" $VERSION
	cd $VERSION
