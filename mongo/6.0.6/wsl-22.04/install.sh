...
if [ ! -d "$HOME/programs/$FOLDER_NAME/$VERSION" ]; then
	...
	wget -q --show-progress "https://fastdl.mongodb.org/linux/mongodb-linux-x86_64-ubuntu2204-$VERSION.tgz"
	printf "\t${bold}${green}Extracting source code${clear}\n"
	tar -xf "mongodb-linux-x86_64-ubuntu2204-$VERSION.tgz"
	...
	if [ -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/mongod" ]; then
		...
		cd ..
		rm "mongodb-linux-x86_64-ubuntu2204-$VERSION.tgz"
