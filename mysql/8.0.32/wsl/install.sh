...
if [ ! -d "$HOME/programs/$FOLDER_NAME/$VERSION" ]; then
	...
	mkdir bld
	cd bld
	cmake .. -DDOWNLOAD_BOOST=1 -DWITH_BOOST=$HOME/programs/$BOOST_FOLDER_NAME -DCMAKE_INSTALL_PREFIX=$HOME/programs/$FOLDER_NAME/$VERSION -DOPENSSL_ROOT_DIR=$HOME/programs/$OPENSSL_FOLDER_NAME/$OPENSSL_VERSION -DBISON_EXECUTABLE=$HOME/programs/$BISON_FOLDER_NAME/$BISON_VERSION/bin/bison -DCURSES_LIBRARY=$HOME/programs/$NCURSES_FOLDER_NAME/$NCURSES_VERSION/lib -DCURSES_INCLUDE_PATH=$HOME/programs/$NCURSES_FOLDER_NAME/$NCURSES_VERSION/include
	make
	sudo make install
