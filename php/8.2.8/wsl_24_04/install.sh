...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/php" ]; then
	...
	bash $INSTALL_FILES_DIR/$LIBXML_FOLDER_NAME/$LIBXML_VERSION/wsl_24_04/install.sh
	bash $INSTALL_FILES_DIR/$OPENSSL_FOLDER_NAME/$OPENSSL_VERSION/wsl_24_04/install.sh
	bash $INSTALL_FILES_DIR/$PKG_CONFIG_FOLDER_NAME/$PKG_CONFIG_VERSION/wsl_24_04/install.sh
	bash $INSTALL_FILES_DIR/$ZLIB_FOLDER_NAME/$ZLIB_VERSION/wsl_24_04/install.sh
	bash $INSTALL_FILES_DIR/$CURL_FOLDER_NAME/$CURL_VERSION/wsl_24_04/install.sh
	bash $INSTALL_FILES_DIR/$SQLITE_FOLDER_NAME/$SQLITE_VERSION/wsl_24_04/install.sh
	bash $INSTALL_FILES_DIR/$ONIGURUMA_FOLDER_NAME/$ONIGURUMA_VERSION/wsl_24_04/install.sh
	bash $INSTALL_FILES_DIR/$AUTOCONF_FOLDER_NAME/$AUTOCONF_VERSION/wsl_24_04/install.sh
	bash $INSTALL_FILES_DIR/$APACHE_FOLDER_NAME/$APACHE_VERSION/wsl_24_04/install.sh
	bash $INSTALL_FILES_DIR/$POSTGRES_FOLDER_NAME/$POSTGRES_VERSION/wsl_24_04/install.sh
	bash $INSTALL_FILES_DIR/$LIBICONV_FOLDER_NAME/$LIBICONV_VERSION/wsl_24_04/install.sh
	bash $INSTALL_FILES_DIR/$M4_FOLDER_NAME/$M4_VERSION/wsl_24_04/install.sh
	bash $INSTALL_FILES_DIR/$UNIXODBC_FOLDER_NAME/$UNIXODBC_VERSION/wsl_24_04/install.sh
	...
	if [ -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/php" ]; then
		...
		printf "\t\t${bold}${green}Downloading source code${clear}\n"
		wget -q --show-progress "https://pecl.php.net/get/redis-$PHP_EXTENSION_REDIS_VERSION.tgz"
		printf "\t\t${bold}${green}Extracting source code${clear}\n"
		...
		wget -q --show-progress "https://getcomposer.org/installer"
