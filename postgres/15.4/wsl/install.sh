...
if [ ! -e $HOME/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/postgresql.conf ]; then
	printf "postgresql.conf not found\n"
	exit
fi

if [ ! -e $HOME/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/pg_hba.conf ]; then
...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/pg_ctl" ]; then
	...
	if [ -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/pg_ctl" ]; then
		...
		mv data/postgresql.conf ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/postgresql.conf.default
		ln -s ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/postgresql.conf data/postgresql.conf
		mv data/pg_hba.conf ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/pg_hba.conf.default
		ln -s ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/pg_hba.conf data/pg_hba.conf

		touch start.sh
		echo 'export LD_LIBRARY_PATH=$HOME'"/programs/$GEOS_FOLDER_NAME/$GEOS_VERSION/lib:"'$LD_LIBRARY_PATH' >> start.sh
		echo "" >> start.sh
		echo 'export LD_LIBRARY_PATH=$HOME'"/programs/$PROJ_FOLDER_NAME/$PROJ_VERSION/lib:"'$LD_LIBRARY_PATH' >> start.sh
		echo "" >> start.sh
		echo 'export LD_LIBRARY_PATH=$HOME'"/programs/$LIBTIFF_FOLDER_NAME/$LIBTIFF_VERSION/lib:"'$LD_LIBRARY_PATH' >> start.sh
		echo "" >> start.sh
...
EOF
		...
		bash $INSTALL_FILES_DIR/$POSTGIS_FOLDER_NAME/$POSTGIS_VERSION/wsl/install.sh $VERSION
	fi
fi
