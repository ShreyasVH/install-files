...
if [ ! -e $HOME/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl_20_04/postgresql.conf ]; then
	printf "postgresql.conf not found\n"
	exit
fi

if [ ! -e $HOME/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl_20_04/pg_hba.conf ]; then
...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/pg_ctl" ]; then
	...
	if [ -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/pg_ctl" ]; then
		...
		mv data/postgresql.conf ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl_20_04/postgresql.conf.default
		ln -s ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl_20_04/postgresql.conf data/postgresql.conf
		mv data/pg_hba.conf ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl_20_04/pg_hba.conf.default
		ln -s ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl_20_04/pg_hba.conf data/pg_hba.conf
...
EOF
		...
		bash $INSTALL_FILES_DIR/$POSTGIS_FOLDER_NAME/$POSTGIS_VERSION/wsl_20_04/install.sh $VERSION
	fi
fi
