...
if [ ! -e $HOME/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/redis.conf ]; then
	printf "redis.conf not found\n"
	exit
fi

if [ ! -e $HOME/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/sentinel.conf ]; then
...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/redis-server" ]; then
	...
	if [ -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/redis-server" ]; then
		...
		touch start.sh
		mv $HOME/sources/$FOLDER_NAME/$VERSION/redis.conf ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/redis.conf.default
		mv $HOME/sources/$FOLDER_NAME/$VERSION/sentinel.conf ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/sentinel.conf.default
		echo "redis-server ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/redis.conf" >> start.sh
		echo "" >> start.sh
		echo "redis-sentinel ~/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl/sentinel.conf" >> start.sh
		echo "" >> start.sh

		touch stop.sh
		echo 'PORT=$(grep '\''^port '\'' ~/workspace/myProjects/config-samples/'"$FOLDER_NAME/$VERSION"'/wsl/redis.conf | awk '\''{print $2}'\'')' >> stop.sh
		echo 'SENTINEL_PORT=$(grep '\''^port '\'' ~/workspace/myProjects/config-samples/'"$FOLDER_NAME/$VERSION"'/wsl/sentinel.conf | awk '\''{print $2}'\'')' >> stop.sh
