...
if [ ! -e $HOME/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl_20_04/rabbitmq.conf ]; then
...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/sbin/rabbitmq-server" ]; then
	...
	bash $INSTALL_FILES_DIR/$ELIXIR_FOLDER_NAME/$ELIXIR_VERSION/wsl_20_04/install.sh
	bash $INSTALL_FILES_DIR/$MAKE_FOLDER_NAME/$MAKE_VERSION/wsl_20_04/install.sh
	...
	ln -s $HOME/workspace/myProjects/config-samples/$FOLDER_NAME/$VERSION/wsl_20_04/rabbitmq.conf ./etc/rabbitmq/
