...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/curl" ]; then
	...
	./configure --prefix=$HOME/programs/$FOLDER_NAME/$VERSION --with-ca-bundle=$HOME/workspace/myProjects/ssl/rootCA.crt --with-openssl=$HOME/programs/$OPENSSL_FOLDER_NAME/$OPENSSL_VERSION --without-libpsl --without-brotli --without-zstd --without-libgsasl > $HOME/logs/$FOLDER_NAME/$VERSION/configureOutput.txt 2>&1
