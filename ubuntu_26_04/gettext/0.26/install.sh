...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/gettext" ]; then
	...
	# ./configure --prefix=$HOME/programs/$FOLDER_NAME/$VERSION --with-included-gettext > $HOME/logs/$FOLDER_NAME/$VERSION/configureOutput.txt 2>&1
	
	# source $INSTALL_FILES_DIR/makeAndInstall.sh $FOLDER_NAME $VERSION $((DEPTH))
