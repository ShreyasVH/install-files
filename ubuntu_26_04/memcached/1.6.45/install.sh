...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/memcached" ]; then
	...
	if [ -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/memcached" ]; then
		...
		touch start.sh
		echo "PORT=\$(grep 'port=' \$HOME/workspace/myProjects/config-samples/$OS/$FOLDER_NAME/$VERSION/memcached.conf | awk -F '=' '{print \$2}')" >> start.sh
		echo '' >> start.sh
		echo 'if ! lsof -i :$PORT > /dev/null; then' >> start.sh
		echo -e '\techo "Starting"' >> start.sh
		echo -e "\tmemcached -p \$PORT -d > memcached.log 2>&1 &" >> start.sh
		echo 'fi' >> start.sh

		touch stop.sh
		echo "PORT=\$(grep 'port=' \$HOME/workspace/myProjects/config-samples/$OS/$FOLDER_NAME/$VERSION/memcached.conf | awk -F '=' '{print \$2}')" >> stop.sh
		echo '' >> stop.sh
