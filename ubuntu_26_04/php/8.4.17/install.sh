...
PHP_EXTENSION_PHALCON_VERSION=$(jq -r --arg folder "$FOLDER_NAME" --arg version "$VERSION" --arg os "$OS" --arg name "$PHP_EXTENSION_PHALCON_FOLDER_NAME" '.[$os][$folder][$version][$name] // .[$folder][$version][$name] // empty' "$STATIC_VERSION_MAP_PATH")
...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/php" ]; then
	...
	wget --show-progress "https://www.php.net/distributions/$ARCHIVE_FILE" > $HOME/logs/$FOLDER_NAME/$VERSION/download.txt 2>&1
