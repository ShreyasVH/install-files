...
PHP_FOLDER_NAME=php
PHP_VERSION=$(jq -r --arg folder "$FOLDER_NAME" --arg version "$VERSION" --arg os "$OS" --arg name "$PHP_FOLDER_NAME" '.[$os][$folder][$version][$name] // .[$folder][$version][$name] // empty' "$STATIC_VERSION_MAP_PATH")
