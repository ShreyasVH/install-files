...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/java" ]; then
	...
	ARCHIVE_FILE=openlogic-openjdk-$VERSION+7-linux-x64.tar.gz
	wget -q "https://builds.openlogic.com/downloadJDK/openlogic-openjdk/$VERSION+7/$ARCHIVE_FILE"
	print_message "${bold}${green}Extracting source code${clear}" $((DEPTH))
	tar -xf $ARCHIVE_FILE
	mv "openlogic-openjdk-$VERSION+7-linux-x64" $VERSION
