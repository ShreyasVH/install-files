...
GETTEXT_VERSION=$(cat "$STATIC_VERSION_MAP_PATH" | jq -r --arg folder "$FOLDER_NAME" --arg version "$VERSION" --arg name "$GETTEXT_FOLDER_NAME" '.[$folder][$version][$name]')
...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/bin/php" ]; then
	...
	print_message "${bold}${yellow}Installing $FOLDER_NAME${clear}" $((DEPTH))
