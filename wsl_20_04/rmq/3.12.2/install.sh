...
if [ ! -e $HOME/workspace/myProjects/config-samples/$OS/$FOLDER_NAME/$VERSION/rabbitmq-env.conf ]; then
	printf "rabbitmq-env.conf not found\n"
	exit
fi
...
if [ ! -e "$HOME/programs/$FOLDER_NAME/$VERSION/sbin/rabbitmq-server" ]; then
	...
	cp $HOME/workspace/myProjects/config-samples/$OS/$FOLDER_NAME/$VERSION/rabbitmq-env.conf ./etc/rabbitmq/
